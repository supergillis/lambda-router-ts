...
const corsDefaults: CorsProps = {
  ...
  headers: ['origin', 'content-type', 'accept', 'authorization', 'cache-control', 'referrer'],
  credentials: true,
  statusCode: 200,
};
...
): Handler<Req, Response> => async (req: Req, context: Context): Promise<Option<Response>> => {
...
function corsPropsToHeaders(req: Request, props: CorsProps) {
  ...
  headers['Access-Control-Allow-Origin'] = props.origin === 'header' ? req.headers['origin'] : props.origin.string;
  headers['Access-Control-Allow-Methods'] = props.methods.join(',');
  headers['Access-Control-Allow-Headers'] = props.headers.join(',');
  headers['Access-Control-Allow-Credentials'] = props.credentials ? 'true' : 'false';
  return headers;
}
